      ...
      <SheetContent className='w-[90%]! max-w-md bg-surface border-l border-outline-variant/30 p-0 flex flex-col z-50 shadow-2xl [&>button]:hidden'>
        ...
        <SheetHeader className='flex flex-row justify-between items-center px-6 py-5 border-b border-outline-variant/20 space-y-0'>
          <div className='flex items-center gap-2'>
            ...
            <SheetTitle className='font-display text-lg font-bold text-on-surface uppercase tracking-wider'>              Your Order
            </SheetTitle>
          </div>
          <button
        ...
        {cart.length === 0 ? (
          ...
          <div className='flex-1 flex flex-col items-center justify-center p-6 text-center space-y-4'>
            ...
            <h4 className='font-display text-lg font-semibold text-on-surface'>              The basket is empty
            </h4>
            <p className='font-sans text-xs text-on-surface-variant max-w-xs mx-auto'>              Choose from our premium specialties and treats to
              add it to the cart.
            </p>
            <button
              onClick={closeCart}
          ...
          <div className='flex-1 flex flex-col overflow-hidden'>
            ...
            <div className='flex-1 overflow-y-auto p-4 md:p-6 space-y-4'>
              <p className='text-[10px] font-sans font-bold uppercase tracking-widest text-primary border-b border-outline-variant/10 pb-2'>                Selected Specialties ({getTotalCount()})
              </p>
                ...
                >
                  ...
                  <div className='flex-1 flex flex-col justify-between'>
                    <div className='flex justify-between items-start gap-1'>
                      ...
                      <span className='font-sans text-xs font-bold text-primary shrink-0'>                        {(item.menuItem.price * item.quantity).toLocaleString()}{' '}
                        day
                      </span>
                    </div>
            ...
            </div>            {/* Select delivery method */}
            <div className='hidden md:block space-y-4 p-6'>
              <label className='text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-2 block'>                Shipping method</label>
            ...
            <div className='bg-surface-container-high p-6 border-t border-outline-variant/20 space-y-4'>
              <div className='space-y-1.5 text-xs font-sans'>
                <div className='flex justify-between text-on-surface-variant'>
                  <span>Amount:</span>
                  <span>{itemsTotal.toLocaleString()} day</span>
                </div>

                <div className='flex justify-between text-on-surface-variant'>
                  <span>Delivery:</span>
                  <span>{deliveryFee.toLocaleString()} day</span>
                </div>

                <div className='flex justify-between text-sm font-bold text-on-surface pt-2 border-t border-outline-variant/15'>
                  <span>Total to pay:</span>
                  <span className='text-primary'>                    {total.toLocaleString()} day
                  </span>
                </div>
              </div>
