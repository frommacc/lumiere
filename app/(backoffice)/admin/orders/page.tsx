      ...
      <div className='space-y-6 px-6 py-8 md:px-10'>
        <form className='flex flex-col gap-3 sm:flex-row'>
          <Input
            ...
            placeholder='Број, име или телефон'
            className='max-w-md bg-surface-container-high'
          />
          <select
            name='status'
            defaultValue={status ?? ''}
            className='h-9 rounded-md border border-outline-variant/30 bg-surface-container-high px-3 text-sm'
          >
        ...
        <div className='overflow-hidden rounded-xl border border-outline-variant/20 bg-surface-container-low/40'>
          <div className='overflow-x-auto'>
            <table className='w-full min-w-230 text-left text-sm'>
              <thead className='border-b border-outline-variant/15 text-[10px] uppercase tracking-[0.16em] text-on-surface-variant'>
                <tr>
                  <th className='px-5 py-4'>Нарачка</th>
                  <th className='px-5 py-4'>Клиент</th>
                  <th className='px-5 py-4'>Износ</th>
                  <th className='px-5 py-4'>Статус</th>
                  <th className='px-5 py-4'>Акции</th>
                </tr>
              </thead>
          ...
          {!orders.length ? (
            <p className='p-12 text-center text-sm text-on-surface-variant'>
              Нема нарачки за избраните филтри.
            </p>
          ) : null}
        </div>
      </div>
